...
<div class="min-h-screen bg-neutral-950 text-white px-6 py-10">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-8">
            <h1 class="text-3xl font-bold text-red-500">Kelola Owner</h1>
            <div class="flex gap-3">
                <a href="{{ route('admin.owner.create') }}" 
                   class="bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white px-5 py-2 rounded-lg font-semibold transition shadow-lg">
                    <i class="fas fa-user-plus mr-2"></i>Tambah Owner
                </a>
            </div>
        </div>
                ...
                <tbody class="divide-y divide-neutral-800">
                    @forelse ($owner as $o)
                        <tr class="hover:bg-neutral-800/40 transition">
                            ...
                            <td class="px-6 py-4 font-medium text-gray-100">{{ $o->nama_lengkap }}</td>
                            <td class="px-6 py-4 text-gray-400">{{ $o->email }}</td>
                            <td class="px-6 py-4 text-gray-300">{{ $o->phone }}</td>
                            <td class="px-6 py-4">
                                @if ($o->status_aktif)
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center gap-2">
                                    <a href="{{ route('admin.owner.edit', $o->id) }}" 
                                       class="bg-yellow-500 hover:bg-yellow-600 text-black px-3 py-1.5 rounded-lg text-xs font-semibold transition">
                                        Edit
                                    </a>
                                    <form action="{{ route('admin.owner.destroy', $o->id) }}" method="POST" 
                                          onsubmit="return confirm('Yakin ingin menghapus owner ini?')" class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
        ...
        {{-- Pagination --}}
        <div class="mt-6">
            {{ $owner->links('pagination::tailwind') }}
        </div>
    </div>
</div>
@endsection
